...
const { numberOfCorrectPasswords, getParsedPassword, sledRentalPlacePasswordPolicy, tobogganPasswordPolicy } = require('../src/2')

const p1 = getParsedPassword('1-3 a: abcde')
const p2 = getParsedPassword('1-3 b: cdefg')
const p3 = getParsedPassword('2-9 c: ccccccccc')

describe('2', () => {

  test('Example case', () => {
    expect(numberOfCorrectPasswords(exampleInput, sledRentalPlacePasswordPolicy)).toBe(2)
  })

  test('getPasswordPolicy splits string and returns expected object', () => {
    expect(p1).toMatchObject({min: 1, max: 3, char: 'a', pass: 'abcde'})
  })

  test('getPasswordPolicy actual input', () => {
    const input = '13-15 v: vvvvvvkvvvvjzvv'
    const policy = getParsedPassword(input)
    expect(policy).toMatchObject({min: 13, max: 15, char: 'v', pass: 'vvvvvvkvvvvjzvv'})
  })

  test('isValidPassword', () => {
    expect(sledRentalPlacePasswordPolicy(p1)).toBe(true)
    expect(sledRentalPlacePasswordPolicy(p2)).toBe(false)
    expect(sledRentalPlacePasswordPolicy(p3)).toBe(true)

  })

  test('actual input', () => {
    const result = numberOfCorrectPasswords(input, sledRentalPlacePasswordPolicy)
    console.log(result)
  })

  test('tobogganPasswordPolicy true example', () => {
    expect(tobogganPasswordPolicy(p1)).toBe(true)
  })

  test('tobogganPasswordPolicy p2 should be false', () => {
    expect(tobogganPasswordPolicy(p2)).toBe(false)
  })

  test('tobogganPasswordPolicy p3 should be false', () => {
    expect(tobogganPasswordPolicy(p3)).toBe(false)
  })

  test('day 2 part 2 actual input', () => {
   const result = numberOfCorrectPasswords(input, tobogganPasswordPolicy)
   console.log(result)
  })
